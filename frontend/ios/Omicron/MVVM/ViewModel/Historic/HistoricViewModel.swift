...
//        dataChipsBinding()
    ...
    func searchDidTapBinding() {
        searchDidTap.withLatestFrom(searchFilter).subscribe(onNext: { [weak self] text in
            print(text)
            self?.getHistoryDataMock(orders: text, offset: self?.dataOffset ?? 0, limit: 10)
    ...
    /*func dataChipsBinding() {
        dataChips.subscribe(onNext: { [weak self] data in
            ...
            self.chips = data
            self.getHistoricData(chips: data, offset: self.offset, limit: self.limit)
            self.getHistoricDataMock(chips: data, offset: 0, limit: 10)
        }).disposed(by: disposeBag)
    }*/
    ...
    /*func bindOnScroll() {
        self.onScroll.subscribe(onNext: {[weak self] _ in
            guard let self = self else { return }
            self.dataOffset += 10
            self.getHistoricDataMockOutdated(orders: String(), offset: self.dataOffset, limit: 10)
        }).disposed(by: disposeBag)
    }*/
    ...
    func getHistoryDataMock(orders: String, offset: Int, limit: Int) {
        ...
        networkManager.getOrdenDetail(225305)
            ...
            .subscribe(onNext: {[weak self] res in
                ...
                let parentOrders = getExampleMock(quantity: 1)
                self.orders.append(contentsOf: parentOrders)
                tableData.onNext(self.orders)
    ...
    func takeElements<T>(from array: [T], startIndex: Int, count: Int) -> [T] {
        guard startIndex >= 0, startIndex < array.count else {
            return [] // si el índice inicial no es válido
        }
        
        let endIndex = min(startIndex + count, array.count)
