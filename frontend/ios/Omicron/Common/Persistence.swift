...
class Persistence {
    ...
    func getLoginData() -> LoginResponse? {
        do {
            let data = try self.userDefaults.getObject(
                forKey: UsersDefaultsConstants.loginData, castTo: LoginResponse.self)
            return data
        } catch {
            print(error.localizedDescription)
            return nil
