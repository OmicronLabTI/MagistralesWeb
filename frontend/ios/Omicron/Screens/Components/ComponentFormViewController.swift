...
class ComponentFormViewController: FormViewController {
    ...
    // MARK: Functions
    func buildForm(component: ComponentO, order: Order) -> Void {
    ...
    func saveChanges () {
        if(self.canOperation(rowValue: self.baseQuantity?.value ?? "f") && self.canOperation(rowValue: (self.requiredQuantity?.value) ?? "r")) {
            ...
            let values = ComponentFormValues(baseQuantity: baseQuantityValue, requiredQuantity: requiredQuantityValue, warehouse: warehouseValue)
